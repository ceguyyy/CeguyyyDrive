...
class ApprovalTemplateService {
    async createTemplate(orgId, name, createdBy, steps) {
        ...
        return await approvalTemplateRepository.createTemplate(orgId, name.trim(), createdBy, steps);
    }
    ...
    async updateTemplate(orgId, templateId, name, steps) {
        ...
        const updated = await approvalTemplateRepository.updateTemplate(orgId, templateId, name.trim(), steps);
        if (!updated) {
